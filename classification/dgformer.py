...
class MFFN(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.mffn = nn.Sequential(
            # pw
            nn.Conv2d(in_features, hidden_features, 1, 1, 0, bias=False),
            nn.BatchNorm2d(hidden_features),
            nn.SiLU(),
            # dw
            nn.Conv2d(hidden_features, hidden_features, 3, 1, 1, groups=hidden_features, bias=False),
            nn.BatchNorm2d(hidden_features),
            nn.SiLU(),
            # pw-linear
            nn.Conv2d(hidden_features, out_features, 1, 1, 0, bias=False),
...
class SPSA(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        self.act = nn.GELU()
        self.sr1 = nn.MaxPool2d(kernel_size=sr_ratio // 2, stride=sr_ratio // 2)
        self.norm1 = nn.LayerNorm(dim)
        self.sr2 = nn.MaxPool2d(kernel_size=sr_ratio, stride=sr_ratio)
        self.norm2 = nn.LayerNorm(dim)
        self.kv1 = nn.Linear(dim, dim, bias=qkv_bias)
        self.kv2 = nn.Linear(dim, dim, bias=qkv_bias)
    ...
    def forward(self, x, H, W):
        ...
        if self.sr_ratio > 1:
            ...
            x_1 = self.act(self.norm1(self.sr1(x_).reshape(B, C, -1).permute(0, 2, 1)))#1,81,64
            x_2 = self.act(self.norm2(self.sr2(x_).reshape(B, C, -1).permute(0, 2, 1)))#1,16,64
            kv1 = self.kv1(x_1).reshape(B, -1, 2, self.num_heads // 2, C // self.num_heads).permute(2, 0, 3, 1, 4)#2,1,2,81,16
            kv2 = self.kv2(x_2).reshape(B, -1, 2, self.num_heads // 2, C // self.num_heads).permute(2, 0, 3, 1, 4)#2,1,2,16,16
            k1, v1 = kv1[0], kv1[1]  # B head N C
            k2, v2 = kv2[0], kv2[1]
            attn1 = (q[:, :self.num_heads // 2] @ k1.transpose(-2, -1)) * self.scale
            attn1 = attn1.softmax(dim=-1)
...
class Dual_Granularity_Former_Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
        ...
        # self.mlp = CFNS(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        # self.mlp = MFFN(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
...
class RelativePositionEncoding(nn.Module):
    def __init__(self, seq_len, hidden_size):
        ...
        # 初始化相对位置矩阵
        pos_matrix = torch.arange(seq_len).unsqueeze(1) - torch.arange(seq_len)
        self.register_buffer('pos_matrix', pos_matrix)

        # 初始化相对位置编码矩阵
        self.weight = nn.Parameter(torch.randn(hidden_size, seq_len))

    def forward(self, x):
        # 计算相对位置编码矩阵
        pos_embed = self.weight @ self.pos_matrix.to(x.device)

        # 将相对位置编码矩阵加入输入特征中
        return x + pos_embed.permute(2, 0, 1).unsqueeze(-1)
...
class Dual_Granularity_Former(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_channels=3, num_classes=1000, embed_dims=64,
                 num_heads=[2, 4, 8, 12], mlp_ratios=[8, 8, 4, 4], qkv_bias=True, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm, act_cfg=nn.GELU,depths=[3, 4, 6, 3],
        ...
        for i in range(num_stages):
            ...
            if i == 0:
                ...
                #     nn.BatchNorm2d(embed_dims_i),
                # )
                patch_embed = ConvStem(in_channels, embed_dims_i)
            else:
            ...
            block = nn.ModuleList()
            patch_sizes = [4,2,2,2]  ## 7,3,3,3 之前
            ...
            block.extend([Dual_Granularity_Former_Block(
                dim=embed_dims_i, num_heads=num_heads[i], mlp_ratio=mlp_ratios[i], qkv_bias=qkv_bias,
                qk_scale=qk_scale,
...
@register_model
def DGFormer_DGA_EFFN(pretrained=False, **kwargs):
    model = Dual_Granularity_Former(
        patch_size=4, embed_dims=16, num_heads=[2, 4, 10, 16], mlp_ratios=[4, 4, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 2, 2], sr_ratios=[8, 6, 4, 2], **kwargs)
